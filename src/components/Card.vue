...
const props = defineProps({
  ...
  tagColorMap: {type: Object, required: true}
});
...
const tagsEdit = (editable:boolean) => {
    ...
    if(editable) {
        nextTick(() => {
            ...
            if(tagsEl.value) {
                const el = tagsEl.value as HTMLInputElement;
                const tags = Array.from(new Set(el.value.split(",").map(tag => tag.trim()).filter(tag => tag)));
...
<div class="relative group overflow-hidden flex flex-col focus:border-indigo-600 select-none min-h-[4em] min-w-[12em] max-w-[24em] max-h-[24em]  place-content-center items-center p-1 m-2   duration-75 ease-linear transition-transform bg-white rounded-md shadow " tabindex="0"
    ...
    <button @click.stop="deleteCard" aria-label="Delete card"
         class="absolute right-0 top-0 p-1 rounded-full font-bold bg-red-500 text-slate-50 focus:bg-red-600  text-xs opacity-0 group-hover:opacity-100 group-focus:opacity-100 hover:opacity-100 focus:opacity-100 outline-none focus:border-red-800">
         <DeleteIcon />
         </button>

    <span class="my-2 py-3 text-xl w-full min-h-[2em] bg-white flex place-content-center items-center overflow-x-hidden border-b border-slate-200"
        @dblclick="edit(true)"
        @focusout="edit(false)"
    ...
    <span class="min-h-[1em] text-xs  flex-grow flex-row w-full  flex-wrap gap-1 "
        ...
        @touchend="tagTap"
        >
        <span class="px-2 py-1 bg-slate-800 text-slate-50 inline-flex rounded border shadow-sm"
            :class="{[`${props.tagColorMap[tag as string]}`]: true}"
            v-if="props.card.tags && !state.tagsEditing" v-for="tag in props.card.tags">{{tag}}</span>
        <input
            class="shadow-sm text-md font-mono p-1 ring-2 ring-indigo-600 outline-none bg-neutral-50 block w-full sm:text-sm border-gray-300 rounded-sm"
...
[contenteditable="true"] {
    @apply rounded-sm bg-neutral-50 outline-none;
}
</style>